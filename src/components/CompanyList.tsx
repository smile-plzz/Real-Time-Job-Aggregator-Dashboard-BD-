...
export default function CompanyList({ companies, onScrape, bulkScraping, onBulkScrape, onStopBulkScrape }: CompanyListProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-xs" id="company-directory-section">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2.5">
          <button
            ...
            className="px-4 py-2 bg-gray-100 hover:bg-gray-100 text-gray-900 border border-gray-300 rounded-xl text-xs font-semibold flex items-center justify-center gap-2 shadow-xs disabled:opacity-45 disabled:hidden transition-all cursor-pointer"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-600 font-medium">Total Companies</span>
            <Building2 className="w-4 h-4 text-indigo-600" />
          </div>
          <div className="mt-2.5">
            <div className="text-xl font-bold text-gray-900">{totalListed}</div>
            <p className="text-[10px] text-gray-500 mt-0.5">Bangladeshi Tech Directory</p>
          </div>
        </div>

        <div className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-600 font-medium">Synced Portals</span>
          ...
          <div className="mt-2.5">
            <div className="text-xl font-bold text-gray-900 flex items-center gap-1.5">
              {totalScraped}
              {totalScraping > 0 && (
                <span className="text-xs text-indigo-600 font-normal animate-pulse flex items-center gap-0.5">
                  ({totalScraping} active)
                </span>
              )}
            </div>
            <p className="text-[10px] text-gray-500 mt-0.5">Direct Crawlers Executed</p>
          </div>
        </div>

        <div className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-600 font-medium">Discovered Jobs</span>
            <Sparkles className="w-4 h-4 text-violet-400 animate-pulse" />
          </div>
          <div className="mt-2.5">
            <div className="text-xl font-bold text-gray-900">{totalLiveJobs}</div>
            <p className="text-[10px] text-gray-500 mt-0.5">Real-time parsed software roles</p>
          </div>
        </div>

        <div className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-600 font-medium">Crawl Accuracy / Success</span>
            <Layers className="w-4 h-4 text-amber-600" />
          </div>
          <div className="mt-2.5">
            <div className="text-xl font-bold text-gray-900">{successRate}%</div>
            <p className="text-[10px] text-gray-500 mt-0.5">Direct Fetch Success Ratio</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 mb-6" id="directory-filters">
        ...
        <div className="relative sm:col-span-2 lg:col-span-2">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm placeholder-slate-500 text-gray-900 focus:outline-hidden focus:border-indigo-500/50 focus:bg-gray-100/30 transition-all"
          />
        </div>
        ...
        <div>
          <select
            ...
            className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-xl text-xs sm:text-sm text-gray-800 focus:outline-hidden focus:border-indigo-500/50 focus:bg-gray-100/30 transition-all cursor-pointer"
          >
      ...
      <div className="overflow-x-auto border border-gray-200 rounded-xl" id="directory-table">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-white/50 border-b border-gray-200 text-gray-600 text-xs font-mono uppercase tracking-wider">
              ...
              {currentCompanies.length > 0 ? (
                currentCompanies.map((company, index) => (
                  <motion.tr
                    ...
                    className="hover:bg-gray-100/40 text-gray-800 text-sm"
                  >
                    {/* Name & Location */}
                    <td className="py-4 px-4">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {company.size && company.size !== 'Please update' && (
                          <span className="inline-flex items-center text-[10px] font-bold text-indigo-600 bg-indigo-500/5 border border-indigo-500/15 px-1.5 py-0.5 rounded-md">
                            {company.size} Team
                          </span>
                        )}
                      </div>
                      <div className="text-xs text-gray-500 mt-1 max-w-sm truncate" title={company.location}>
                      ...
                      {company.technologies && company.technologies.length > 0 && (
                        <div className="flex flex-wrap items-center gap-1 mt-2 max-w-sm">
                          {company.technologies.slice(0, 4).map((tech, idx) => (
                            <span key={`${tech}-${idx}`} className="text-[10px] text-gray-600 bg-gray-100/60 px-1.5 py-0.5 rounded-md border border-gray-300/30 font-mono">
                              {tech}
                            </span>
                          ))}
                      ...
                      <div className="flex items-center gap-3 text-gray-600">
                        ...
                        {company.contact ? (
                          <div className="flex items-center gap-1 relative group">
                            <Users className="w-4 h-4 text-gray-500 hover:text-indigo-600 cursor-pointer transition-colors" />
                            <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover:block w-48 bg-gray-100 border border-gray-300 rounded-lg shadow-xl p-2 z-10">
                              <div className="text-[10px] font-bold text-gray-800 mb-1 border-b border-gray-300 pb-1">HR / Recruiter Contacts</div>
                                ...
                                {company.contact.split(';').map((contact, idx) => {
                                  ...
                                  return (
                                    <a 
                                      ...
                                      target="_blank" 
                                      rel="noopener noreferrer"
                                      className="text-[10px] text-indigo-600 hover:text-indigo-300 truncate"
                                    >
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-2">
                        ...
                        {(!company.scrapeStatus || company.scrapeStatus === 'idle') && (
                          <span className="inline-flex items-center text-xs text-gray-600 font-medium px-2 py-1 rounded-full bg-gray-100">
                            Unscanned
                          </span>
                        )}

                        {company.scrapeStatus === 'scraping' && (
                          <span className="inline-flex items-center text-xs text-indigo-600 font-semibold px-2.5 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 gap-1.5 animate-pulse">
                        ...
                        {company.scrapeStatus === 'completed' && (
                          <div className="flex flex-col gap-0.5">
                            <span className="inline-flex items-center text-xs text-emerald-600 font-semibold px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 gap-1 self-start">
                        ...
                        {company.scrapeStatus === 'failed' && (
                          <div className="flex flex-col gap-0.5">
                            <span className="inline-flex items-center text-xs text-rose-600 font-semibold px-2.5 py-1 rounded-full bg-rose-500/10 border border-rose-500/20 gap-1 self-start" title={company.error}>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {company.career && (
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-gray-100 text-gray-800 hover:bg-gray-100 hover:text-indigo-600 border border-gray-300 hover:border-indigo-500/30 rounded-lg text-xs font-semibold flex items-center gap-1 transition-all cursor-pointer disabled:opacity-40"
                        ...
                        <button
                          ...
                          disabled={company.scrapeStatus === 'scraping'}
                          className="px-2.5 py-1.5 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-600 border border-indigo-500/20 rounded-lg text-xs font-semibold flex items-center gap-1 transition-all cursor-pointer disabled:opacity-40"
      ...
      {(totalPages > 1 || itemsPerPage === 'all' || filteredCompanies.length > 8) && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-5 pt-4 border-t border-gray-200" id="directory-pagination">
          <div className="flex flex-wrap items-center gap-4">
            <p className="text-xs text-gray-500 font-medium">
              Showing <span className="font-semibold text-gray-800">{filteredCompanies.length > 0 ? indexOfFirstItem + 1 : 0}</span> to{' '}
              <span className="font-semibold text-gray-800">
                {Math.min(indexOfLastItem, filteredCompanies.length)}
              </span>{' '}
              of <span className="font-semibold text-gray-800">{filteredCompanies.length}</span> companies
            </p>

            <div className="flex items-center gap-2 text-xs text-gray-500">
              <span className="text-[11px] text-gray-500 font-medium">Page limit:</span>
                  ...
                  return (
                    <button
                      ...
                      className={`px-2.5 py-1 text-[10px] font-semibold rounded-md transition-all cursor-pointer ${
                        isActive
                          ? 'bg-indigo-600/20 text-indigo-600 border border-indigo-500/30'
          ...
          {itemsPerPage !== 'all' && totalPages > 1 && (
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="px-3 py-1.5 bg-gray-100 hover:bg-gray-100 disabled:opacity-40 text-gray-800 rounded-lg text-xs font-medium cursor-pointer transition-colors border border-gray-300"
              >
                Previous
              </button>
