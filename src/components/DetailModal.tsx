...
import { 
  ...
  Globe, Mail, Users, AlertCircle, RefreshCw
} from 'lucide-react';
...
export function DetailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto bg-slate-950/70 backdrop-blur-md animate-in fade-in duration-200">
      ...
      {/* Modal Shell */}
      <div 
        className="relative w-full max-w-4xl bg-white rounded-3xl border border-slate-200/80 shadow-2xl overflow-hidden my-auto max-h-[92vh] flex flex-col z-10 animate-in zoom-in-95 duration-200"
        onClick={e => e.stopPropagation()}
      >
        
        {/* Modern Minimalist Glass Header */}
        <div className="px-6 py-5 bg-slate-900 text-white flex items-start justify-between gap-4 border-b border-slate-800 shrink-0">
          <div className="flex items-start gap-3.5">
            {icon ? (
              <div className="p-2.5 bg-indigo-500/20 text-indigo-300 rounded-2xl border border-indigo-500/30 shrink-0 mt-0.5">
                {icon}
              </div>
            ) : (
              <div className="p-2.5 bg-indigo-500/20 text-indigo-300 rounded-2xl border border-indigo-500/30 shrink-0 mt-0.5">
        ...
        {/* Company Profile Header Banner (if applicable) */}
        {companyProfile && (
          <div className="px-6 py-4 bg-gradient-to-r from-indigo-50/90 via-slate-50 to-indigo-50/90 border-b border-indigo-100/80 flex flex-wrap items-center justify-between gap-4 text-xs shrink-0">
            <div className="flex items-center gap-3.5">
              <div className="w-11 h-11 rounded-2xl bg-indigo-600 text-white font-black flex items-center justify-center text-lg shadow-md ring-2 ring-indigo-200">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-slate-600 text-[11px] mt-1 flex-wrap">
        ...
        {statsSummary && statsSummary.length > 0 && (
          <div className="px-6 py-3.5 bg-slate-50/80 border-b border-slate-200/80 grid grid-cols-2 sm:grid-cols-4 gap-3 shrink-0">
            {statsSummary.map((stat, idx) => (
              <div key={idx} className="bg-white p-3 rounded-2xl border border-slate-200/70 shadow-2xs">
                <div className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500">{stat.label}</div>
        ...
        <div className="px-6 py-3.5 bg-white border-b border-slate-200 flex flex-col sm:flex-row items-center gap-3 shrink-0">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all"
            />
            {searchQuery && (
              <button 
          ...
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto shrink-0">
            ...
            <select
              ...
              className="bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-2 text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer"
            >
            ...
            {(searchQuery || selectedExp !== 'all' || selectedCategory !== 'all' || selectedType !== 'all') && (
              <button
                ...
                className="px-2.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs transition-colors flex items-center gap-1 cursor-pointer"
                title="Reset Filters"
              >
          ...
          {filteredJobs.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-3xl border border-slate-200 p-8 shadow-2xs space-y-4">
              ...
              <div className="pt-2">
                <button
                  onClick={handleResetFilters}
                  className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs transition-all shadow-sm cursor-pointer inline-flex items-center gap-2"
                >
            ...
            filteredJobs.map(job => {
              ...
              return (
                <div 
                  key={job.id} 
                  className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/90 hover:border-indigo-400 shadow-2xs hover:shadow-md transition-all space-y-3"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h4 className="text-base font-extrabold text-slate-900 hover:text-indigo-600 transition-colors">
                          {job.title}
                        </h4>
                        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-100 uppercase">
                          {job.category}
                        </span>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="px-4 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs transition-all flex items-center gap-1.5 cursor-pointer shadow-xs"
                      >
                        <span>Apply</span>
                  ...
                  {isExpanded && (
                    <div className="pt-3 border-t border-slate-100 text-xs text-slate-700 leading-relaxed bg-slate-50/80 p-3.5 rounded-xl space-y-2 animate-in fade-in duration-150">
